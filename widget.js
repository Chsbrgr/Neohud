...
window.addEventListener('onSessionUpdate', function (obj) {
	Object.assign(sessionData, obj.detail);
	updateCSSVars();
	updateFeed();
});
...
function updateCSSVars() {
	const r = document.querySelector(':root');
	// Set Follower Goal Width
	let followerGoalWidth
	//Adding switch to allow Follow Goal Width based on goal type, also changed to "let" to allow changes to variable
	switch (`${fieldData.followerGoalType}`) {
		case 'goal':
			followerGoalWidth = (getFormatedNumber(sessionData[`follower-${fieldData.followerGoalType}`].amount) / `${fieldData.followerGoalAmount}`) * 100 + '%'; //Changed Const to Let
			break;
		default:
			followerGoalWidth = (getFormatedNumber(sessionData[`follower-${fieldData.followerGoalType}`].count) / `${fieldData.followerGoalAmount}`) * 100 + '%';
	}
	// Set Subcriber Goal width
	let subscriberGoalWidth
	switch (`${fieldData.subscriberGoalType}`) {
		case 'goal':
			subscriberGoalWidth = (getFormatedNumber(sessionData[`subscriber-${fieldData.subscriberGoalType}`].amount) / `${fieldData.subscriberGoalAmount}`) * 100 + '%'; //Changed Const to Let
			break;
		default:
			subscriberGoalWidth = (getFormatedNumber(sessionData[`subscriber-${fieldData.subscriberGoalType}`].count) / `${fieldData.subscriberGoalAmount}`) * 100 + '%';
	}
...
function addTwitchLabels() {
	...
	const fields = ['Latest', 'Top', 'Count', 'Goal', 'Session', 'Week', 'Month'];
...
function addSocialLabels() {
	const plattforms = ['twitch', 'youtube', 'tiktok', 'instagram', 'twitter', 'facebook', 'discord'];
...
function getLatestLabel(type) {
	...
	content += getIcon(fieldData[`${type}Icon`], fieldData[`${type}IconType`]);
...
function getGoalLabel(type) {
	...
	let progress = ''
	switch (true) {
		case (type === 'subscriber' && fieldData['subscriberGoalType'] === 'goal'):
			progress = getFormatedNumber(sessionData[`subscriber-${fieldData.subscriberGoalType}`].amount);
			break;
		case type === 'subscriber':
			progress = getFormatedNumber(sessionData[`subscriber-${fieldData.subscriberGoalType}`].count);
			break;
		case (type === 'follower' && fieldData['followerGoalType'] === 'goal'):
			progress = getFormatedNumber(sessionData[`follower-${fieldData.followerGoalType}`].amount);
			break;
		case type === 'follower':
			progress = getFormatedNumber(sessionData[`follower-${fieldData.followerGoalType}`].count);
			break;
		default:
			progress = getFormatedNumber(sessionData[`${type}-goal`].amount);
...
function getSocialLabel(plattform) {
	...
	content += getIcon(plattform, 'brands');
	content += '<div class="text-wrapper"><div class="text">';
...
function getIcon(name = '', type = 'solid') {
	if (!name) return '';

	return `<i class="fa-${type} fa-${name} icon"></i>`;
}
...
function getMessageFromTemplate(type, data) {
	...
	for (const [key, value] of Object.entries(data)) {
		const template = `{${key}}`;
		message = message.split(template).join(value);
	}
